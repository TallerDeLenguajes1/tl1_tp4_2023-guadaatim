...
void eliminarNodo(nodo ** start, nodo * nodoeliminar);
...
void eliminarNodo(nodo ** start, nodo * nodoeliminar)
{
    ...
    while (aux && aux->t->tareaID != nodoeliminar->t->tareaID)
    {
        auxanterior = aux;
        aux = aux->siguiente;
    }
